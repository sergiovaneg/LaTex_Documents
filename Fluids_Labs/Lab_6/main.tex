\title{CFD laboratory 1\\Flow around a circular cylinder in the sub-critical regime}
\author{
        Sergio M. Vanegas A.\\
        Francesco de Pas\\
...
Concerning the drag and lift coefficients evolution over time we used the  \mathcal{0.06}\:s timestep and the numerical results are plotted  in Figure~\ref{fig:drag_coeff} and  Figure~\ref{fig:lift_coeff}. At a first sketch they seem consistent with the phisical phenomenon, since the lift coefficient oscillations are \mathcal{0} averaged, while drag coefficients oscillations are positively averaged. 
